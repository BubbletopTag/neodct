...
#
# The check itself has moved to neodct/scripts/check-users.sh, and the reason
# is in that file: it was the last thing this script did, this script runs for
# the QEMU target only, and the LUCKFOX image -- the one that goes on the phone
# -- was therefore the one with no guard at all. Both defconfigs now name the
# standalone script directly in BR2_ROOTFS_POST_IMAGE_SCRIPT.
#
# It is still run from here so that a QEMU build fails at the same point it
# always did, whichever order buildroot happens to run the two scripts in;
# running it twice costs one grep.
"$SCRIPT_DIR/check-users.sh"
