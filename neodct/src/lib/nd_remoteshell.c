...
/* Is there a key here, rather than just a file?
 *
 * ssh-keygen creates the file before it has any key material to put in it, so
 * an interrupted run leaves a zero-byte file behind -- and on a first boot it
 * can be interrupted for a while, because generating a key wants entropy the
 * kernel has not gathered yet. Asking only whether the file EXISTS turns that
 * into a phone that can never start sshd again: "no hostkeys available", every
 * boot, for good, while the tunnel comes up and holds and the person trying to
 * log in sees nothing but a refused connection. */
static bool host_key_usable(void)
{
    char resolved[ND_PATH_MAX];
    struct stat st;
    const char *argv[7];

    if (nd_path_resolve(resolved, sizeof resolved, ND_RS_HOST_KEY) != ND_OK)
        return false;
    if (stat(resolved, &st) != 0 || !S_ISREG(st.st_mode))
        return false;
    if (st.st_size <= 0)
        return false; /* the common case, answered without a fork */

    /* And the bytes have to be a key openssh can load, because a phone loses
     * power and ssh-keygen is not atomic: a file that stops halfway through
     * fails in sshd exactly as an empty one does. `-y` prints the public half
     * of a private key and fails when it cannot read one; `-P ""` so a file
     * that is not a key cannot turn into a passphrase prompt on a stdin
     * nobody is holding. */
    argv[0] = ND_RS_KEYGEN;
    argv[1] = "-y";
    argv[2] = "-P";
    ...
    argv[5] = resolved;
    argv[6] = NULL;
    return run_tool(argv, NULL, 0u) == 0;
}
...
nd_err nd_rs_ensure_host_key(char *errmsg, size_t n)
{
    ...
    /* Take the unusable pair out of the way first: ssh-keygen refuses to
     * overwrite a file it finds and asks on stdin, which nobody is holding. */
    (void)unlink(key);
...
{
    /* The tunnel has always been a retry loop because mobile data drops. sshd
     * needs one for a smaller and more embarrassing reason: it is started at
     * boot, before the network scripts have run, and binding 127.0.0.1 fails
     * with "Address not available" when loopback is not up yet. It exited, and
     * nothing started it again -- while the tunnel came up and held, so the
     * phone looked reachable right up to the moment a connection arrived at a
    ...
    words[n_words++] = ND_RS_SSHD;
    words[n_words++] = "-f";
    words[n_words++] = config;
    words[n_words++] = "-D"; /* the loop has to have something to wait on */
    words[n_words++] = "-e"; /* and its reasons belong in the log */
    ...
    for (i = 0u; i < n_words; i++) {
        ...
        if (nd_rs_quote(quoted, sizeof quoted, words[i]) != ND_OK)
            return ND_ERR_TOOLONG;
        if (i > 0u && append(line, sizeof line, " ") != ND_OK)
            return ND_ERR_TOOLONG;
        if (append(line, sizeof line, quoted) != ND_OK)
            return ND_ERR_TOOLONG;
    }
    ...
    rc = append(text, sizeof text,
                ...
                "while :; do\n");
    if (rc == ND_OK) {
        char indented[RS_TEXT_MAX];

        (void)nd_snprintf(indented, sizeof indented, "    %s\n", line);
        rc = append(text, sizeof text, indented);
    }
    if (rc == ND_OK) {
        char ended[128];

        (void)nd_snprintf(ended, sizeof ended,
                          "    echo \"[RSHELL] sshd ended ($?); retrying in %d\"\n",
                          ND_RS_RETRY_SECONDS);
        rc = append(text, sizeof text, ended);
    }
    if (rc == ND_OK) {
        char naps[32];

        (void)nd_snprintf(naps, sizeof naps, "    sleep %d\n", ND_RS_RETRY_SECONDS);
        rc = append(text, sizeof text, naps);
    }
    if (rc == ND_OK)
        rc = append(text, sizeof text, "done\n");
    if (rc != ND_OK)
    ...
    return write_file_mode(ND_RS_SSHD_SCRIPT, text, 0700);
}
...
{
    nd_rs_settings current;
    char sshd_script[ND_PATH_MAX];
    char script[ND_PATH_MAX];
    char log_path[ND_PATH_MAX];
    char pidtext[32];
    const char *sshd_argv[3];
    ...
    rc = nd_rs_write_sshd_script();
    if (rc != ND_OK) {
        say(errmsg, n, "Could not write sshd.sh.");
        return rc;
    }
    ...
    if (nd_path_resolve(sshd_script, sizeof sshd_script, ND_RS_SSHD_SCRIPT) != ND_OK ||
    ...
    /* The LOOP, not sshd itself. Remote Shell comes up at boot before the
     * network scripts have run, and sshd binding 127.0.0.1 fails outright
     * when loopback is not up yet -- once, and then never again, while the
     * tunnel retried its way to a working connection and made the phone look
     * reachable. */
    sshd_argv[0] = "/bin/sh";
    sshd_argv[1] = sshd_script;
    sshd_argv[2] = NULL;
    rc = spawn_session("/bin/sh", sshd_argv, log_fd, &pid);
    if (rc != ND_OK) {
        (void)close(log_fd);
    ...
    (void)write_file_mode(ND_RS_SSHD_LOOP_PID, pidtext, 0600);
...
nd_err nd_rs_stop(nd_rs_status *out, bool remember)
{
    ...
    /* The loop BEFORE the sshd it supervises: kill sshd first and the loop
     * just starts another one. */
    stop_pid(ND_RS_SSHD_LOOP_PID, "sshd.sh");
