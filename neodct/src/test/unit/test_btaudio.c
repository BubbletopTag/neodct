...
/* The route S17audio writes with the debug link up is longer than
 * ND_BTAUDIO_CONF_MAX. It used to be saved through a buffer that size and
 * came back truncated, which is no audio at all after the earbuds go. */
static void test_a_long_boot_route_survives_the_round_trip(void)
{
    char boot[1400];
    char back[sizeof boot];
    size_t i;

    (void)snprintf(boot, sizeof boot, "# Generated by /etc/init.d/S17audio.\n");
    for (i = strlen(boot); i < sizeof boot - 2u; i++)
        boot[i] = (i % 60u) ? 'x' : '\n';
    boot[i++] = '\n';
    boot[i] = '\0';
...
int main(void)
{
    ...
    RUN(test_a_long_boot_route_survives_the_round_trip);
