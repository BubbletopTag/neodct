...
/* The emulator this tree actually builds now. It is a THIRD record and not a
 * replacement for the one above, because both image tags are real: the
 * aarch64 one is stamped into every QEMU image ever flashed and is still the
 * key its updates are published under. What this pair pins is that the image=
 * key is the only thing that differs -- one word further along the same row
 * of platform-id.sh -- so a new image tag is a table entry and never a code
 * change here. */
#define QEMU_ARMV7_RECORD                                            \
    "# Generated by post-build-system-metadata.sh -- do not edit.\n" \
    "platform=qemu\n"                                                \
    "board=qemu-virt\n"                                              \
    "image=qemu-armv7\n"
...
/* The armv7 emulator, through the real record rather than through
 * platform-id.sh alone -- and paired with an ND_PLATFORM_QEMU build constant,
 * because after the ABI collapse this pairing is the ONLY thing that
 * separates this image from a phone. A mismatch here would be the whole
 * discriminator failing, and it would fail silently: uname(2) says armv7l on
 * both, so nothing else in the process would notice. */
static void test_the_armv7_emulator_is_the_emulator(void)
{
    given_a_platform_record(QEMU_ARMV7_RECORD);
    ...
    /* And a hardware binary that finds this record is a mis-assembled image,
     * exactly as it is for the older tag: the image= key changed, the word
     * the constant is compared against did not. */
...
int main(void)
{
    ...
    RUN(test_the_armv7_emulator_is_the_emulator);
