...
static void test_scrollbar_every_index(nd_capture *cap, nd_ui *ui)
{
    ...
    nd_appsel_draw(&s);
    {
         ...
         * Bluetooth makes, 82 with the twenty-five Sleepy makes, 80 with
         * the twenty-six Calendar makes, and 78 with the twenty-seven OsmAnd
...
/* Engineering mode off is fifteen apps, so the step becomes 99/14 = 7.071 and
 ...
 *
 * It was thirteen and 99/12 until Calendar shipped, and fourteen and 99/13
 * until OsmAnd did. Both counts are the
...
static void test_engineering_off_geometry(nd_capture *cap, nd_ui *ui)
{
    ...
    CHECK_INT(n, 15, "fifteen stock apps with engineering off");
    if (n < 2u)
        return;
