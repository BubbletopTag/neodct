...
### U-15. `service.c` is deliberately NOT wired to any of this — SUPERSEDED

*This was written while the signature half was missing, and both halves have since
landed. It is kept because the rule it states is still the rule.*

The original entry: `apps/Update/service.c` still answers `ND_UPDSVC_UNAVAILABLE`
everywhere. The install path checks compatibility BEFORE the signature — three Python
tests depend on that order — and the signature half does not exist yet, so wiring only
the reader would put the app in a state where a package opens, its platform is checked,
and then the signature check says "unavailable". The boundary stays honest until both
halves are there.

Both halves are there. `service.c` is now the thin adapter it was meant to be, and the
four `remote.*` entry points went the same way once `lib/nd_remote.c` landed — see the
`nd_remote.c` section at the end of this file. `ND_UPDSVC_UNAVAILABLE` is no longer
reachable from any of them, and `update_app.h` gained `ND_UPDSVC_NETWORK` so that a
dropped carrier is not reported as "this build cannot do updates".

---

## The update downloader (`lib/nd_remote.c`)

*Port of `System/core/UpdateService/remote.py` (321 lines) — the only part of the
update system that touches the network. It stops at "there is a file on the card";
`nd_signing.c` decides whether that file is trustworthy.*

`neodct/tests/remote/version_order.txt` is 625 `candidate|installed` pairs and the
answer `remote.is_newer()` actually gave for each, generated by running it; the C is
checked against every one. Every `latest()` and `all_releases()` expectation in
`test_remote.c` was likewise read off `remote.py` run against the committed fixture
bytes — the recipe is in `neodct/tests/remote/README.md`.

**No HTTP request has ever been made from this code.** Not in a test and not by hand.
Everything below the process boundary — that GitHub answers the way the fixtures do,
that TLS verifies against the phone's CA bundle, that the real curl accepts this argv —
is unproven and is listed as such in `test_remote.c`'s header.

### G-1. The transport is a spawned `curl`, not linked `libcurl`

`spec-update-system.md` §6 assumes `libcurl` in `PKG_DEPS` and a transport vtable.
Neither is here. Linking OpenSSL into `libneodct.so` for the signature verifier already
cost **+1.3 MB of idle RSS in every process that maps the library**, because `nd-core`
maps it at boot and never verifies a signature; libcurl and a second TLS stack would be
the same bargain again, for a subprocess that runs a few times a year. `curl`,
`libcurl` and `ca-certificates` are already in both defconfigs and the owner has run
`curl` on the device.

The cost is that nothing fails at build time if curl goes missing — no unresolved
symbol, no import to fail, just "cannot reach GitHub: no curl on this phone" the next
time somebody looks for an update. That is exactly how `aplay` went missing for six
weeks, so `neodct/tests/test_update_curl.py` pins the three defconfig options, the two
installed files, and that `PKG_DEPS` still does **not** name curl.

### G-2. There is no `nd_remote_set_transport()`

The spec asked for one so the 36 Python tests could be reproduced behind a seam. With
the transport in another executable, **PATH is the seam**: `neodct/tests/remote/fake-curl`
goes first on PATH and the library finds it with its own PATH walk. Nothing in the
library is stubbed, compiled differently or given a test-only branch, which is a
stronger arrangement than a vtable — the argv, the pipes, the header parsing and the
resume arithmetic are all the shipped ones.

`nd_remote_set_sleep_fn()` does exist, for the same reason `remote._sleep` is
monkeypatched: a backoff of 5 + 10 + 20 seconds in a unit test.

### G-3. An asset URL that is not plainly `https` is skipped

`remote.py` copies `browser_download_url` out of the JSON unexamined; the C requires
`https://`, printable ASCII, no quotes or angle brackets, and under `ND_REMOTE_URL_MAX`.
An entry that fails is skipped as though it carried no asset for this platform.

Two reasons. The URL becomes one `argv` entry of a spawned program, so a value
beginning with `-` would be read by curl as an option (there is no shell anywhere in
this path, so this is the only place such a thing could get in). And a rootfs must not
be fetched over plaintext — `--proto '=https'` would refuse it a moment later anyway.

The direction is "offer an older release", never "offer a worse one".
`test_an_insecure_asset_url_is_skipped()` asserts the divergence against a fixture
`remote.py` was run over, so it cannot drift back.

### G-4. A field that does not fit its bound makes the release unusable, not truncated

Same rule as `nd_manifest.c`'s U-3, for the same reason: a truncated URL fetches the
wrong thing and a truncated version is compared against the installed one. `notes` is
the single exception and is truncated at `ND_REMOTE_NOTES_MAX` — losing the tail of a
paragraph costs nothing and refusing a release over its prose would be absurd.

### G-5. `version_key` has three bounds Python does not

Components past the eighth are dropped, a suffix past 15 characters is truncated, and a
number past `INT64_MAX` saturates instead of growing a bignum. Each can only make two
keys compare **equal** that Python would have ordered, and `is_newer()` answers false on
equal — so the effect is "not offered", never "wrongly offered". No published version
comes near any of them; the largest is three components of two digits.

### G-6. A repository override that cannot go in a URL is refused

`NEODCT_UPDATE_REPO` is checked against `[A-Za-z0-9._/-]{1,127}` and a value that fails
produces a network error naming the variable, rather than a differently-shaped request.
Python would build the URL and let GitHub answer 404, which reads as "nothing
published" — a mistyped override should not look like an empty repository.

### G-7. `--proto-redir '=https'` refuses a downgrade `urllib` would follow

Python's `urlopen` follows an `https` → `http` redirect. The C does not. GitHub's asset
URLs redirect to `objects.githubusercontent.com` over https, so this costs nothing
today; it means an attacker who could influence the redirect target cannot make the
phone fetch its rootfs in the clear and rely on the signature check alone.

### G-8. Completeness is checked even when curl reported an error

`remote.py` can only fail before the first byte — its network errors come out of
`_open()`. curl fails *during* a transfer, and exit 18 ("transfer closed with bytes
remaining") is what a server that closed right after the last byte looks like. So the
download loop renames the file when `done == size` whatever curl thought of the
connection. Without that branch the package would sit complete on the card while every
remaining attempt asked for `bytes=<size>-`, got nothing, and the download failed with
the finished file beside it. The length decides; the sha256 that follows decides whether
the length was telling the truth.

### G-9. A dropped connection mid-stream is retried here and is a traceback in Python

Worth the owner's attention as a **Python bug this port does not reproduce**.
`_fetch_into()` wraps only `_open()`. A connection reset while `response.read()` is
running raises `http.client.IncompleteRead`, which is an `HTTPException` and not an
`OSError`, so neither of `download()`'s two `except` clauses catches it and it
propagates out of the update flow. The Python's own tests never see this because they
model a dropped carrier as a *short read* rather than an exception. The C treats it as
a `NetworkError`, keeps the bytes and retries — which is what the docstring and the
tests both describe as the intended behaviour.

### G-10. `prerelease` and `size` must be the JSON types they look like

`bool(entry.get("prerelease"))` is true for any non-empty string; `nd_json_get_bool()`
accepts only a real JSON boolean. `int(asset.get("size") or 0)` would raise on a
non-numeric string; the C reads 0, which downstream means "size unknown" and disables
...
which `staging.c` already writes from the manifest. The hash tree itself is built at
image-build time by `neodct/tools/mknand.sh`. The phone never constructs one.
