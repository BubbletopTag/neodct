...
Platform comparison is an exact byte comparison. Real values in the tree: `qemu-armv7`,
`luckfox-armv7` (and `qemu-aarch64` on images flashed before the emulator moved to
armv7). They come from `system.os.platform` in
`/NeoDCT/System/version.prop`, generated by `post-build-system-metadata.sh` from
`BR2_ROOTFS_POST_BUILD_SCRIPT_ARGS`.

The C has since grown **one** accepted exception to the byte comparison, which the
Python reference does not have: an image whose platform is `qemu-aarch64` accepts a
package built for `qemu-armv7`. It is a table of ordered pairs with that one entry, and
the reverse is refused — an `.ndsw` carries no kernel, so an aarch64 rootfs under the armv7 kernel now
running would `switch_root` into a filesystem with no `init` it can execute. Nothing on
the download side knows about it: `nd_remote_asset_name()` builds one name from one
platform string and never looks for the other's.

**The exception is half a migration, not a rescue.** The comparison runs in the running
image's libneodct, so an image flashed before the rename is executing the older bare
`strcmp` and refuses the armv7 package however it arrives — the alias is in a rootfs it
cannot reach. What reaches it is one transitional build,
`BR2_ROOTFS_POST_BUILD_SCRIPT_ARGS="... qemu-aarch64"` on the armv7 defconfig, which
`platform-id.sh` still maps and which the old `strcmp` therefore accepts; the image that
comes up carries the alias and one `qemu-armv7` package finishes the move. Until such a
release is cut, those images stay where they are.
