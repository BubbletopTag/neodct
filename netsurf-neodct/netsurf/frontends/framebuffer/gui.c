...
widget_scroll_y(struct gui_window *gw, int y, bool abs)
{
	...
	if (gw->vscroll != NULL)
		fbtk_set_scroll_position(gw->vscroll,
...
widget_scroll_x(struct gui_window *gw, int x, bool abs)
{
	...
	if (gw->hscroll != NULL)
		fbtk_set_scroll_position(gw->hscroll,
				bwidget->scrollx + bwidget->panx);
}

/* send a synthetic left click to the page at screen coordinates */
void
fb_browser_click_at(struct gui_window *gw, int sx, int sy)
{
	struct browser_widget_s *bwidget = fbtk_get_userpw(gw->browser);
	int x = sx - fbtk_get_absx(gw->browser) + bwidget->scrollx;
	int y = sy - fbtk_get_absy(gw->browser) + bwidget->scrolly;

	if (sy < fbtk_get_absy(gw->browser))
		return;
...
const char *
fb_browser_link_at(struct gui_window *gw, int sx, int sy)
{
	struct browser_widget_s *bwidget = fbtk_get_userpw(gw->browser);
	...
	if (browser_window_get_features(gw->bw, x, y,
					&features) != NSERROR_OK) {
		return NULL;
	}
...
static nserror set_defaults(struct nsoption_s *defaults)
{
	...
	/* NeoDCT view size unless overridden */
	if (nsoption_int(window_width) <= 0)
		nsoption_set_int(window_width, 240);
	if (nsoption_int(window_height) <= 0)
		nsoption_set_int(window_height, 175);

	/* NeoDCT memory budget.
	 *
	 * These are desktop defaults otherwise, and on this phone they are
	 * the difference between a page loading and the OOM killer taking
	 * the browser. The stock content cache alone is 12 MB -- a fifth of
	 * the machine -- and it is why an ordinary page can leave netsurf
	 * holding 29 MB of anonymous memory while the launcher is squeezed
	 * down to 1.6 MB resident with 17.5 MB pushed into zram.
	 *
	 * 2 MB still holds a page's images comfortably at 240x175, which is
	 * the only size anything is ever drawn at here.
	 */
	nsoption_set_int(memory_cache_size, 2 * 1024 * 1024);

	/* 24 parallel fetches is a desktop on fibre. This is one core
	 * behind a GPRS modem, and every fetcher in flight is buffers and a
	 * curl handle held for the duration. Fewer is both less memory and,
	 * on a link this thin, not measurably slower.
	 */
	nsoption_set_int(max_fetchers, 4);
	nsoption_set_int(max_fetchers_per_host, 2);

	/* NeoDCT fonts: the system font (Nokia style) is carried by the
	 * fantasy/cursive faces so only the chrome and pages that ask
	 * for it use it; web content gets a regular font (DejaVu).
	 * NEODCT_FONT overrides the system font for host development. */
	{
		static const char *dejavu_sans[] = {
			"/usr/share/fonts/dejavu/DejaVuSans.ttf",
			"/usr/share/fonts/TTF/DejaVuSans.ttf",
			"/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",
			NULL
		};
		static const char *dejavu_sans_bold[] = {
			"/usr/share/fonts/dejavu/DejaVuSans-Bold.ttf",
			"/usr/share/fonts/TTF/DejaVuSans-Bold.ttf",
			"/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",
			NULL
		};
		static const char *dejavu_serif[] = {
			"/usr/share/fonts/dejavu/DejaVuSerif.ttf",
			"/usr/share/fonts/TTF/DejaVuSerif.ttf",
			"/usr/share/fonts/truetype/dejavu/DejaVuSerif.ttf",
			NULL
		};
		static const char *dejavu_mono[] = {
		...
		for (i = 0; dejavu_sans[i] != NULL; i++) {
			if (access(dejavu_sans[i], R_OK) == 0) {
				web_sans = dejavu_sans[i];
				web_sans_bold = dejavu_sans_bold[i];
				web_serif = dejavu_serif[i];
				web_mono = dejavu_mono[i];
				break;
			}
		}

		/* a face must never be missing: fall back across fonts */
		if (web_sans == NULL)
			web_sans = sysfont;
		if (sysfont == NULL)
			sysfont = web_sans;
		if (web_sans_bold == NULL || access(web_sans_bold, R_OK) != 0)
			web_sans_bold = web_sans;
		if (web_serif == NULL || access(web_serif, R_OK) != 0)
			web_serif = web_sans;
		if (web_mono == NULL || access(web_mono, R_OK) != 0)
			web_mono = web_sans;

		if (web_sans != NULL) {
			nsoption_setnull_charp(fb_face_sans_serif,
					       strdup(web_sans));
			nsoption_setnull_charp(fb_face_sans_serif_bold,
					       strdup(web_sans_bold));
			nsoption_setnull_charp(fb_face_sans_serif_italic,
					       strdup(web_sans));
			nsoption_setnull_charp(fb_face_sans_serif_italic_bold,
					       strdup(web_sans_bold));
			nsoption_setnull_charp(fb_face_serif,
					       strdup(web_serif));
			nsoption_setnull_charp(fb_face_serif_bold,
					       strdup(web_serif));
			nsoption_setnull_charp(fb_face_monospace,
					       strdup(web_mono));
			nsoption_setnull_charp(fb_face_monospace_bold,
					       strdup(web_mono));
		}
		if (sysfont != NULL) {
			nsoption_setnull_charp(fb_face_cursive,
					       strdup(sysfont));
			nsoption_setnull_charp(fb_face_fantasy,
					       strdup(sysfont));
		}
	}
...
fb_browser_window_input(fbtk_widget_t *widget, fbtk_callback_info *cbi)
{
	...
	/* NeoDCT chrome consumes all key input */
	if (gw->neodct != NULL) {
		return neodct_shell_input(gw, cbi);
	}
...
fb_update_back_forward(struct gui_window *gw)
{
	...
	if (gw->back == NULL || gw->forward == NULL)
		return;
...
/* NeoDCT window: slim chrome from the neodct shell, no stock furniture */
static void
create_neodct_browser_window(struct gui_window *gw)
{
	NSLOG(netsurf, INFO, "NeoDCT window");

	gw->window = fbtk_create_window(fbtk, 0, 0, 0, 0, 0);

	/* browser widget under the url bar, no scrollbars */
	create_browser_widget(gw, NEODCT_URLBAR_H, 0);

	neodct_shell_create(gw, feurl);
...
resize_normal_browser_window(struct gui_window *gw, int furniture_width)
{
	...
	if (gw->toolbar == NULL) {
		/* NeoDCT window: fixed chrome, just track the surface */
		if (fbtk_set_pos_and_size(gw->window, 0, 0, 0, 0)) {
			resize_browser_widget(gw, 0, NEODCT_URLBAR_H,
					fbtk_get_width(gw->window),
					fbtk_get_height(gw->window) -
					NEODCT_URLBAR_H);
		}
		return;
	}
		...
		gui_window_create_flags flags)
{
	...
	(void)create_normal_browser_window;
	create_neodct_browser_window(gw);
...
{
	if (g->status == NULL) {
		/* NeoDCT chrome drives its own status text */
		return;
	}
...
{
	if (g->neodct != NULL) {
		neodct_shell_set_hover(g, shape == GUI_POINTER_CARET);
	}
...
{
	if (g->neodct != NULL) {
		neodct_shell_set_url(g, nsurl_access(url));
		return NSERROR_OK;
	}
