...
/* NeoDCT: the placeholder geometry, so a <video> defaults to a box that
 * fits the panel. Pure geometry, no frontend dependencies. */
...
/**
 * NeoDCT: width and height for a <video>.
 *
 * Same shape as the canvas hint, but defaulting to the panel rather than
 * the 300x150 the spec gives a canvas: a default-sized video box would be
 * wider than the screen, and one element overflowing pushes every other
 * thing on the page sideways out of reach.
 */
static void css_hint_height_width_video(
		nscss_select_ctx *ctx,
		dom_node *node)
{
	struct css_hint *hint = &hint_ctx.hints[hint_ctx.len];
	...
	if (set_dim == false) {
		hint->prop = CSS_PROP_HEIGHT;
		hint->data.length.unit = CSS_UNIT_PX;
		hint->data.length.value = INTTOFIX(NEODCT_MEDIA_DEFAULT_H);
		hint->status = CSS_HEIGHT_SET;
		css_hint_advance(&hint);
	} else {
		set_dim = false;
	...
	if (set_dim == false) {
		hint->prop = CSS_PROP_WIDTH;
		hint->data.length.unit = CSS_UNIT_PX;
		hint->data.length.value = INTTOFIX(NEODCT_MEDIA_DEFAULT_W);
		hint->status = CSS_WIDTH_SET;
		css_hint_advance(&hint);
	}
}
		...
		uint32_t *nhints, css_hint **hints)
{
	...
	case DOM_HTML_ELEMENT_TYPE_VIDEO:
		css_hint_height_width_video(pw, node);
		break;
