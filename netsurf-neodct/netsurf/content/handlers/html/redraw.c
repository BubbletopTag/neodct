...
/* NeoDCT: where the play button goes inside a <video> placeholder. Pure
 * geometry with no frontend types in it. */
...
/**
 * NeoDCT: draw the placeholder that stands in for a <video>.
 *
 * White on black, hard edges, no antialiasing: the panel is 240x175 and a
 * softened shape at this size reads as a smudge. The triangle is stacked
 * one-pixel rectangles rather than a polygon for the same reason -- it is
 * pixel art, and it should look drawn rather than rendered.
 *
 * \param ctx    current redraw context
 * \param x      left edge of the content box
 * \param y      top edge of the content box
 * \param width  content width in device pixels
 * \param height content height in device pixels
 * \return true on success, false on error
 */
static bool neodct_redraw_media(const struct redraw_context *ctx,
				int x, int y, int width, int height)
{
	static const char label[] = "VIDEO";
	...
	/* the screen the video would have played on */
	rect.x0 = x;
	rect.y0 = y;
	rect.x1 = x + width;
	rect.y1 = y + height;
	if (ctx->plot->rectangle(ctx, plot_style_fill_black,
				 &rect) != NSERROR_OK) {
		return false;
	}

	neodct_media_layout(width, height, &l);
	if (l.button_size <= 0) {
		return true;
	}

	for (row = 0; row < l.button_size; row++) {
		int w = neodct_media_triangle_row(row, l.button_size,
						  l.button_size);
		if (w <= 0) {
			continue;
		}
		rect.x0 = x + l.button_x;
		rect.y0 = y + l.button_y + row;
		rect.x1 = rect.x0 + w;
		rect.y1 = rect.y0 + 1;
		if (ctx->plot->rectangle(ctx, plot_style_fill_white,
					 &rect) != NSERROR_OK) {
			return false;
		}
	}
	...
	if (l.show_label) {
		...
		if (guit->layout->width(&fstyle, label, sizeof(label) - 1,
					&text_w) == NSERROR_OK) {
			text_x += (width - text_w) / 2;
		}
		/* label_y is the top of the line; text plots from a
		 * baseline, so drop by most of the line height */
		if (ctx->plot->text(ctx, &fstyle, text_x,
				    y + l.label_y + l.label_h - 2,
				    label, sizeof(label) - 1) != NSERROR_OK) {
			return false;
		}
	}
		...
		const struct redraw_context *ctx)
{
	...
	* for BOX_TEXT it's in an inline */
	...
	} else if (box->flags & NEODCT_MEDIA) {
		/* NeoDCT: a <video>. There is no object to draw, because
		 * nothing was fetched -- the url is played, not loaded. */
		if (!neodct_redraw_media(ctx, x + padding_left,
					 y + padding_top, width, height)) {
			return false;
		}
