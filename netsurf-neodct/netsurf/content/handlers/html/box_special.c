...
/* NeoDCT: <video> becomes a play button rather than fallback text. The
 * module is pure geometry and string handling with no frontend types in
 * it, so including it here costs the core nothing. */
...
/**
 * NeoDCT: the url a <video> should play.
 *
 * Prefers the element's own src, then the first <source> child that has
 * one. There is no codec table to choose between them with, and the page
 * author put the one they meant first. Returns a new reference, or NULL
 * when the element names nothing playable.
 */
static nsurl *box_video_src(dom_node *n, html_content *content)
{
	dom_node *child, *next;
	dom_string *s;
	dom_exception err;
	...
	if (err == DOM_NO_ERR && s != NULL) {
		bool ok = box_extract_link(content, s, content->base_url,
					  &url);
		dom_string_unref(s);
		if (ok && url != NULL) {
			return url;
	...
	while (child != NULL) {
		...
		if (err == DOM_NO_ERR && type == DOM_ELEMENT_NODE &&
			...
			err = dom_element_get_attribute(child,
					corestring_dom_src, &s);
			if (err == DOM_NO_ERR && s != NULL) {
				bool ok = box_extract_link(content, s,
...
/**
 * NeoDCT: box construction for a <video> element.
 *
 * NetSurf cannot decode video and this phone could not play the result if
 * it could, so the element becomes an empty replaced box that the redraw
 * path fills with a play button. The url goes on as an ordinary link, so
 * hover, the status bar and clicking all behave exactly as they do for an
 * <a>; the shell asks neodct_media before following one.
 *
 * Children are dropped. They are the fallback content, and "your browser
 * does not support the video tag" is the one thing on the page that this
 * build has made untrue.
 *
 * \param n      dom node of the element
 * \param content the html content being converted
 * \param box    box for the element
 * \param convert_children set false so the fallback text is not shown
 * \return true on success, false on memory exhaustion
 */
static bool box_video(dom_node *n, html_content *content, struct box *box,
		      bool *convert_children)
{
	...
	url = box_video_src(n, content);
	if (url == NULL) {
		/* Nothing to play: leave the fallback content in place so
		 * the page still says something where the video would be. */
		return true;
	}

	/* Remembered as well as linked: a src need not carry an extension
	 * to recognise it by, and <video src="/stream"> is ordinary. */
	neodct_media_remember(nsurl_access(url));
	...
	box->flags |= IS_REPLACED | REPLACE_DIM | NEODCT_MEDIA;
			 ...
			 bool *convert_children)
{
	...
	case DOM_HTML_ELEMENT_TYPE_VIDEO:
		res = box_video(node, content, box, convert_children);
		break;
